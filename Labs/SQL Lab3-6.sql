...
SELECT rentals_june.customer_id, count(rentals_june.rental_id) as count, customer.first_name as FirstName, customer.last_name as LastName
FROM sakila.rentals_june
JOIN customer ON rentals_june.customer_id = customer.customer_id
group by customer_id order by count desc